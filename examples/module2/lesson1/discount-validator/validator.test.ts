...
describe('Form validation', () => {
  ...
  test('values was send correctly to the validator', () => {
    const errors = formValidator('John', 'Doe', 30);
    expect(errors).toEqual([]);
  });

  test('should return an error if first name is empty', () => {
    const errors = formValidator('', 'Doe', 30);
    expect(errors).toContain('First name have to be longer than 0');
  });

  test('should return an error if last name is empty', () => {
    const errors = formValidator('John', '', 30);
    expect(errors).toContain('Last name have to be longer than 0');
  });

  test('should return an error if age is not a number', () => {
    const errors = formValidator('John', 'Doe', '30' as unknown as number);
